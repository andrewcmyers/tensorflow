...
public class SessionTest {
  ...
  public void runUsingOperationNames() {
    try (Graph g = new Graph();
      ...
      try (Tensor<Integer> x = Tensor.create(new int[][] {{5}, {7}}, BaseType.INT32);
  ...
  public void runUsingColonSeparatedNames() {
    try (Graph g = new Graph();
      ...
      try (Tensor<Integer> fed = Tensor.create(new int[] {4, 3, 2, 1}, BaseType.INT32);
          Tensor<Integer> fetched =
              s.runner().feed("Split:0", fed).feed("Split:1", fed).fetch("Add").run().get(0).expect(BaseType.INT32)) {

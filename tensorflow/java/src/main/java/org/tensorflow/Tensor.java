 ...
 * Appropriate types T are declared in {@link org.tensorflow.Types}.
 *
...
public final class Tensor<T> implements AutoCloseable {
   ...
   * Types}). Thus, not all Java objects can be converted to a Tensor. In particular, {@code obj}
   ...
   * Tensor<TFInt64> s = Tensor.create(42L, TFInt32.T);
   *
   * // Valid: A 3x2 matrix of floats.
   * float[][] matrix = new float[3][2];
   * Tensor<TFFloat> m = Tensor.create(matrix, TFFloat.T);
   *
   ...
   * twoD[0] = new int[1];
   * twoD[1] = new int[2];
   * Tensor<TFInt32> x = Tensor.create(twoD, TFInt32.T);
   * }</pre>
   *
   ...
   *     system.
   */
  public static <T> Tensor<T> create(Object obj, Class<T> baseType) {
  	DataType dt1 = dataTypeOf(obj);
  	DataType dt2 = Types.dataType(baseType);
   ...
   * @return this
   */
 @SuppressWarnings("unchecked") public <U> Tensor<U> expect(Class<U> type) {
